...
- Current local install runs on stable v0.107.1 at `mods/damage-forecast`, with DLL SHA256 `8BC96C07DB047F963940D1378A0257F101B886C949368F23F4FAD1C41B0CDF49` and manifest SHA256 `FF8D4E07E574F9FC89EDEDF0D569EE8A7CADFE2A6A2907CAA9E3097F476C32DB`; the same DLL bytes are present in the current stable and beta guardrail outputs.
- Runtime depth: stable v0.107.1 full migration/rollback/re-upgrade matrix PASS; beta v0.109.0 matching-artifact, config continuity, one-load/one-page/one-HUD smoke PASS; the prior stable matching artifact passed the damaging Status/Curse hand matrix, supported/direct combinations, and Block/no-Block scenes with a fresh one-load/no-attributable-error log. The native covering-screen fix is RuntimeVerified on both stable and beta for ordinary combat, map, draw pile, discard pile, and exhaust pile with immediate restore after close.
