...
| FeelNoPainPower + current-hand Ethereal exhaust | Implemented, RuntimeVerified, Conditional | Uses ordered future Block from exact native `FeelNoPainPower.Amount` and deterministic native Ethereal exhaust. Default `-N` includes it; positive `N` includes it only when Power Block is selected. Pending Stampede with a playable Attack remains Unknown before auto-play. Stable/beta L1/L2 passed; L3-01–12 have aggregate cross-target coverage, not a single-target `12/12` claim. |
...
- Current local install is the MP-1R2 diagnostic build on the locally selected stable `v0.107.1 / 59260271` at `mods/damage-forecast`, with DLL SHA256 `8600EEE449646D4AFBDD9E053BA8081AC75A9B9D4F15E43F3AA249B73425B6A8` and manifest SHA256 `FF8D4E07E574F9FC89EDEDF0D569EE8A7CADFE2A6A2907CAA9E3097F476C32DB`; the active tree is strictly the two manifest/DLL files. This diagnostic artifact is `Installed / HashVerified / RuntimeNotVerified`; its recovery and scope authority is `docs/task-notes/multiplayer-client-enemy-death-forecast-snapshot-master-task-card.md`.
